...
function showTransactions() {
  ...
  transactions.forEach((transaction) => {
    let selectedtransaction=JSON.stringify(transaction);
    let tablerow = `<tr>
            <td>${transaction.price}</td>
            <td>${transaction.type}</td>
            <td>${transaction.date}</td>
            <td>${transaction.notes}</td>
            <td><button type="button" onclick="deleteTransaction(${transactions.indexOf(transaction)})">delete</button></td>
        </tr>`;
...
let filterTransactionsbyPrice = (minprice, maxprice) => {
  ...
  return transactions.filter(
    (transaction) =>
      transaction.price >= minprice && transaction.price <= maxprice
  );
};
